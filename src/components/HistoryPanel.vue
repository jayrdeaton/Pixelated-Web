  ...
  <div v-if="history.length > 0" class="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm dark:shadow-none mb-6">
    ...
    <div class="flex gap-3 overflow-x-auto pb-2 px-3 pt-3 -mx-3 bg-gray-50 dark:bg-zinc-950 rounded-xl">
      <div
        ...
        :key="i"
        class="flex-shrink-0 bg-white dark:bg-zinc-900 border-2 border-gray-200 dark:border-zinc-800 rounded-xl p-3 flex flex-col items-center gap-2 w-36 shadow-sm dark:shadow-none cursor-pointer transition-all duration-150 hover:shadow-md hover:scale-[1.03] hover:border-violet-400 dark:hover:border-violet-500"
        @click="downloadItem(item)"
      >
        <div
          class="w-28 h-28 flex items-center justify-center overflow-hidden rounded-lg cursor-zoom-in"
...
import { ref } from 'vue'
import { usePixelate, type HistoryItem } from '@/composables/usePixelate'
